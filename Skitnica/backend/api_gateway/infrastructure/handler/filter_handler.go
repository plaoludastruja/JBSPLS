...
func NewFilterHandler(accomodationClientAddress, appointmentClientAddress, reservationClientAddress, hostmarkClientAddress string) Handler {
...
func (handler *FilterHandler) Init(mux *runtime.ServeMux) {
	err := mux.HandlePath("GET", "/accomodation/filter/{location}/{guestNumber}/{startDay}/{startMonth}/{startYear}/{endDay}/{endMonth}/{endYear}/{wifi}/{parking}/{airConditioning}/{bestHost}/{minPrice}/{maxPrice}", handler.Filter)
	if err != nil {
		panic(err)
	}
}

func (handler *FilterHandler) Filter(w http.ResponseWriter, r *http.Request, pathParams map[string]string) {
	...
	resAfterFilter := []domain.SearchResult{}
	...
	minPrice, _ := strconv.Atoi(pathParams["minPrice"])
	maxPrice, _ := strconv.Atoi(pathParams["maxPrice"])
	hostmarkClient := service.NewHostMarkClient(handler.hostmarkClientAddress)

	for _, searchRes := range res {
		if strings.Contains(searchRes.Facilities, pathParams["wifi"]) && strings.Contains(searchRes.Facilities, pathParams["parking"]) && strings.Contains(searchRes.Facilities, pathParams["airConditioning"]) && (searchRes.TotalPrice >= int32(minPrice)) && (searchRes.TotalPrice <= int32(maxPrice)) {
			if pathParams["bestHost"] == "true" {
				reservationsResult, _ := reservationClient.IsHostBestHost(context.TODO(), &reservation.IsHostBestHostRequest{HostUsername: searchRes.HostUsername})
				hostmarkResult, _ := hostmarkClient.IsHostBestHost(context.TODO(), &hostmark.IsHostBestHostRequest{HostUsername: searchRes.HostUsername})
				if reservationsResult.IsBestHost == true && hostmarkResult.IsBestHost == true {
					resAfterFilter = append(resAfterFilter, searchRes)
				}
			} else {
				resAfterFilter = append(resAfterFilter, searchRes)
			}
	...
	response, _ := json.Marshal(resAfterFilter)
